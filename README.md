...
without a human in the loop. Sensors watch the repo — a commit landing, a coverage gap, a
stale dependency pin, a statistical anomaly in simulated outcomes — and publish typed events
to a SQLite bus. A deterministic orchestrator (zero LLM calls) routes each event to one of
four workers by a static table. Each worker runs `claude-agent-sdk` inside its own git
worktree under a write scope it cannot escape, and nothing reaches `main` except through a
gate that checks scope, then re-runs the full test suite, then fast-forwards. Start it with
`python -m agents.cli up`; watch it with `status` and `events`; stop it with `stop`.

The agents' commits are in this repository's history above the template's `Initial commit`.
As of the last run **7 agent commits have landed** — 6 from `reviewer`, 1 from `test-author` — including a
review that diagnosed a real unbounded loop in the reviewer's own trigger, and which is why
that loop is now fixed. **Read "Status: where this actually is" below before continuing the
work** — two of the four agents have not yet landed anything, and the reason is a specific,
fixable scheduling bug rather than anything deep.

---

## Status: where this actually is

Measured from `agents/state/runs.jsonl` and `git log`, not from intent.
